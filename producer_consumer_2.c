...
void call_back (int argu) {
	int  	index ;
	///信号回调函数 , 当线程没有持有锁的时候 , 如果收到取消信号 , 线程自己退出
...
void * manage_thread (void *data) {
	...
	while (1) {
		///当任务队列中的任务大于100 时 , 动态创建线程加入到数组中
		if (task_queue_hr.task_count > 10 ) {
			if ( (thread_count < 7) ) {
		...
		} else if (task_queue_hr.task_count < 5 ) {
			if (thread_count  > 5) {
